...
uchar const code TX_ADDRESS[TX_ADR_WIDTH]= {0x34,0x43,0x10,0x10,0x01};	//本地地址
uchar const code RX_ADDRESS[RX_ADR_WIDTH]= {0x34,0x43,0x10,0x10,0x01};	//接收地址
...
void NRF24L01_TxPacket(uchar * tx_buf)
{
	...
	SPI_RW_Reg(WRITE_REG + CONFIG,0x5e);   		                     // IRQ收发完成中断响应，16位CRC，主发送
	CE=1;		                //置高CE，激发数据发送
	Delay_us(100);				  //在下一次操作前需要延时一段时间，10us即可	
 	//while(IRQ == 1);
