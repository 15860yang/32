...
/************配置1*********横屏显示*****************************/
	...
	cam_mode.cam_height = 240 - LINE(1);
	
	cam_mode.lcd_sx = 0;
	cam_mode.lcd_sy =  LINE(1);
...
static void FIFO_GPIO_Config(void){
    ...
    /*(FIFO_DATA--FIFO输出数据)*/
		GPIO_InitStructure.GPIO_Pin = 	OV7725_DATA_4_GPIO_PIN | 	OV7725_DATA_5_GPIO_PIN |
    ...
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
		GPIO_Init(OV7725_DATA_GPIO_PORT2, &GPIO_InitStructure);
...
{
	//这里的管脚定义少了A8，A8是无用管脚，还少了HREF的定义，这个是行同步信号，没用到，因为有了VSYNC场(帧)同步信号了
	
	//配置SCCB的两个口为C6C7   也就是SCL(PC6)、SDA(PC7)
	SCCB_GPIO_Config();
	/*开启  oe(a3)  wrst(c4)  rrst(a2)  rclk(c5)  we(d3)  data(b8 - b15) 时钟*/
	FIFO_GPIO_Config();
	//c3
	VSYNC_GPIO_Config();
}
...
void ImagDisp(uint16_t sx,uint16_t sy,uint16_t width,uint16_t height)
{
	...
	char pictureData[224*2+1];
	char headData[20];
	pictureData[224*2] = 0;
	ILI9341_OpenWindow(sx,sy,width,height);
	ILI9341_Write_Cmd ( CMD_SetPixel );	

	sprintf(headData,"*#*#%d#%d#",width,height);
	wifiSendString(headData);
	...
	for(i = 0; i < width; i++)
	{
		for(j = 0; j < height; j++)
		{
			...
			pictureData[j*2] = Camera_Data & 0x00ff;
			pictureData[j*2+1] = (Camera_Data>>8) & 0x00ff;
			
			//pictureData[j*2] = (1) & 0x00ff;
			//pictureData[j*2+1] = ((1)>>8) & 0x00ff;
			
			if(pictureData[j] == 0){
				pictureData[j]++;
			}
			if(pictureData[j*2+1] == 0){
				pictureData[j*2+1]++;
			}
		...
		wifiSendString(pictureData);
	}
	wifiSendString("##");
}

void ov7725Prepare(void){
	uint8_t retry = 0;
