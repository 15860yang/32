...
void initLcdMode(OV7725_MODE_PARAM camMode){
...
void ImagDisp(uint16_t sx,uint16_t sy,uint16_t width,uint16_t height){
	...
	sprintf(btData,"****%d#%d#",width,height);
	HC05_SendString(btData);
	...
	for(i = 0; i < width; i++)
	{
		for(j = 0; j < height; j++)
		{
			...
			//realData[0] = Camera_Data;
			//realData[1] = Camera_Data>>8;
			//HC05_SendString(realData);
			temp = i * height + j;
			realData[0] = temp & 0x00ff;
			realData[1] = (temp>>8) & 0x00ff;
			//realData[0] = 12345;
			//realData[1] = (12345)>>8;
	...
	ILI9341_Clear(sx,sy,width,height);
	HC05_SendString("##");
}
